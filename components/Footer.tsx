    ...
    <footer className="bg-primary pt-20 pb-10 text-white/80 border-t border-white/10">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="lg:col-span-1">
            <a href="#" className="flex items-center gap-2 mb-6">
              <div className="w-10 h-10 rounded-xl bg-accent flex items-center justify-center">
                <svg viewBox="0 0 32 32" className="w-6 h-6 stroke-white">
          ...
          <div>
            ...
            <ul className="space-y-4">
              ...
              <li><a href="#why-us" className="hover:text-accent transition-colors">{t("nav.whyUs") as string}</a></li>
              <li><a href="#reviews" className="hover:text-accent transition-colors">{t("nav.reviews") as string}</a></li>
            </ul>
          </div>

          {/* Contacts */}
          <div>
            <h4 className="text-white font-bold mb-6 uppercase tracking-wider text-sm">{t("footer.contacts") as string}</h4>
            <ul className="space-y-4">
              <li className="flex items-start gap-3">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5 text-accent shrink-0 mt-0.5">
                  <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z" strokeLinecap="round" strokeLinejoin="round"/>
                  <circle cx="12" cy="10" r="3" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                <span>{t("footer.address") as string}</span>
              </li>
              <li className="flex items-center gap-3">
                ...
                <a href={`mailto:${t("footer.email")}`} className="hover:text-white transition-colors">{t("footer.email") as string}</a>
              </li>
              <li className="flex items-center gap-3">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5 text-accent shrink-0">
                  <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 16.92z" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                <a href={`tel:${t("footer.phone")}`} className="hover:text-white transition-colors font-bold text-white">{t("footer.phone") as string}</a>
              </li>
            </ul>
          </div>
          ...
          {/* Social / Extra */}
          <div>
            <h4 className="text-white font-bold mb-6 uppercase tracking-wider text-sm">Графік роботи</h4>
            <p className="mb-2">Пн-Нд: 08:00 - 20:00</p>
            <p className="text-accent text-sm mb-6">Приймаємо термінові замовлення</p>
        ...
        <div className="pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-4 text-sm">
          <p>© {currentYear} Прибирання Карпати (E2E). {t("footer.rights") as string}</p>
